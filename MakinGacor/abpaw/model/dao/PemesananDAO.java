...
public class PemesananDAO {
    ...
    public boolean updateOffline(PemesananOffline pesan) {
        String sql = "UPDATE pemesanan_offline SET id_dokter=?, id_pemilik=?, id_pet=?, nomor_antrean=?, tanggal_antrean=?, waktu_antrean=?, keluhan=?, status_antrean=?, estimasi_waktu=?, biaya_jasa=?, total_biaya=?, status_pembayaran=? WHERE id_antrean=?";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, pesan.getIdDokter());
            stmt.setInt(2, pesan.getIdPemilik());
